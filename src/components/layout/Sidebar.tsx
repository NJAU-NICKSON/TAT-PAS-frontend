...
export function Sidebar({ open = false, onClose }: SidebarProps) {
  ...
  return (
    <>
      {/* Backdrop: only on mobile/tablet when the drawer is open */}
      <div
      ...
      <aside
        className={`fixed left-0 top-0 h-screen flex flex-col transition-transform duration-200 lg:translate-x-0 ${
          open ? 'translate-x-0' : '-translate-x-full'
        }`}
        style={{ width: '248px', zIndex: 50, background: '#FAFAF9', borderRight: '1px solid #EBEBE8' }}
      >
        <div className="flex items-center gap-2.5 px-4 flex-shrink-0"
          ...
          <button
            onClick={onClose}
            className="lg:hidden p-1.5 rounded-md hover:bg-black/5 flex-shrink-0"
            style={{ color: '#8A8A82' }}
